...
let subconfig = "https://raw.githubusercontent.com/.../ACL4SSR_Online_Full_MultiMode.ini";
let socks5Address = '';
let RproxyIP = 'false';
关键变量说明：
变量	类型	说明
...
设计要点：
1.	环境变量优先级最高，方便部署时动态修改
2.	通过 Upgrade 头区分 WebSocket 和普通请求
3.	利用 url.pathname 实现路由分发
4.3 普通 HTTP 请求处理
javascript
...
重试策略：
1.	首次连接使用 VLESS 头中的目标地址
2.	如果连接成功但无数据返回，使用 proxyIP 重试
3.	支持 SOCKS5 代理作为备选路由
六、协议解析机制
6.1 UUID 验证
javascript
...
1.	注册 Cloudflare 账号
2.	创建 Worker
o	进入 Cloudflare Dashboard → Workers & Pages
o	创建新的 Worker
o	粘贴代码
3.	配置环境变量
bash
...
SUBAPI = "订阅转换后端"
4.	绑定自定义域名（可选）
o	Worker → Triggers → Custom Domains
o	添加你自己的域名
7.2 客户端配置
V2Ray / Xray 客户端配置：
json
